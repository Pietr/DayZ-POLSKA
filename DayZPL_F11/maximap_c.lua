...
function drawMap()
	if not toggle then
		...
		if choiImg == 1 then
		  dxDrawImage(x,y,hSize,vSize,mapFile,0,0,0,tocolor(R,G,B,A),false)
		else
		  dxDrawImage(x,y,hSize,vSize,mapFile2,0,0,0,tocolor(R,G,B,A),false)
		end
			
		if myImage then
		dxDrawImage(x,y,hSize,vSize,mapFileGride,0,0,0,tocolor(R,G,B,A),false)
		end
...
function toggleMap()
	...
	local somMap = playSound("sounds/map.mp3")
    setSoundVolume(somMap, 0.3)
...
choiImg = 1
function showClientImageChoise(key, keyState)
 
 if choiImg == 1 then
  choiImg = 2
 elseif choiImg == 2 then
  choiImg = 1
 end
 
end
bindKey("num_5", "down", showClientImageChoise)

myImage = true
function showClientImageTree(key, keyState)
...
bindKey("num_enter", "down", showClientImageTree)
