...
local ICON_DIMENSIONS = 64
local ICON_LINE = 20
local ICON_TEXT_SHADOW = tocolor ( 0, 0, 0, 150 )
...
addEventHandler ( "onClientRender", root,
	function()
		...
		for player in pairs(voicePlayers) do
			local color = tocolor(255, 0, 0, 150)
...
function dxDrawVoiceLabel ( player, index, color )
	...
	dxDrawImage ( sx, sy, icon, icon, ICON_PATH, 0, 0, 0, color, false )

	px = px + spacing

	-- shadows
	--dxDrawText ( getPlayerName ( getLocalPlayer() ), px + 1, py + 1, px, py, ICON_TEXT_SHADOW, scale )
	-- text
	--dxDrawText ( getPlayerName ( getLocalPlayer() ), px, py, px, py, color, scale )
end
