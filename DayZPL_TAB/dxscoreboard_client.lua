...
local todayLogin = nil
local totalLogin = nil


addEventHandler("onClientResourceStart", resourceRoot,
    function()
		triggerServerEvent('askTodayLoginStats',localPlayer)
    end
)
 

function recieveTodayLoginStats (stats, stats_total)
	todayLogin = stats or 0 
	totalLogin = stats_total or 0 
end

addEvent("sendTodayLoginStats",true)
addEventHandler("sendTodayLoginStats",getRootElement(), recieveTodayLoginStats)
	...
	if #scoreboardColumns ~= 0 then
		...
		if (currentTick - scoreboardTicks.lastUpdate > scoreboardTicks.updateInterval and (scoreboardToggled or scoreboardForced)) or forceScoreboardUpdate then
			...
			playerInList = false
				...
				--		playerInList = true
				--	end
				for i, v in pairs ( playerRankingTable ) do
				...
				if getElementData(player,"logedin") then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						elseif column.name == "fps" then
							local fps = (getElementData(player,"FPS") or 0)
								content = tostring(fps)
						elseif column.name == "zkill" then
						local		zkills = (getElementData(player,"zombieskilled") or 0)
								content = tostring(zkills)
						elseif column.name == "kill" then
						local		kills = (getElementData(player,"murders") or 0)
								content = tostring(kills)
						elseif column.name == "pos" then
						content = tostring(i)
						elseif column.name == "alive" then
						local	alivetime = (getElementData(player,"alivetime") or 0)
								content = tostring(formatTimeFromMinutess(alivetime))
						else
					...
					for key, player in ipairs( players ) do
						...
						for key, column in ipairs( scoreboardColumns ) do
							...
							elseif column.name == "ping" then
								content = getPlayerPing( player )
							elseif column.name == "fps" then
							local fps = (getElementData(player,"FPS") or " ")
								content = tostring(fps)
							elseif column.name == "zkill" then
							local	zkills = (getElementData(player,"zombieskilled") or " ")
								content = tostring(zkills)
							elseif column.name == "kill" then
							local	kills = (getElementData(player,"murders") or " ")
								content = tostring(kills)
							--elseif column.name == "pos" then
							--local rank = positionGetElementData("alivetime", #getElementsByType("player"))
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				dxDrawText( "Serwer: " .. serverInfo.server, topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end

			if serverInfo.players and showServerInfo then
				local players = getElementsByType( "player" )
				local text = "Gracze: " .. tostring( #players ) .. "/" .. serverInfo.players.."/ Dziś: "..tostring(todayLogin).."/ Ogółem: "..tostring(totalLogin)
...
addEventHandler( "doScoreboardSetSortBy", getResourceRootElement(),
	function ( name, desc )
		scoreboardSetSortBy( alive, desc )
	end
)
...
function onClientResourceStart ( resource )
	...
	addEventHandler ( "onClientRender", getRootElement (), renderCheckingFPS )
end
addEventHandler ( "onClientResourceStart", getResourceRootElement (), onClientResourceStart )

function renderCheckingFPS ( )
	if ( getTickCount() < FPSTime ) then
		...
		FPSCalc = 0
		FPSTime = getTickCount() + 1000
		setElementData(getLocalPlayer(),"FPS",LastFPS)
	end
end
--------------------------------------
--------------------------------------
...
checkTopPlayer()
setTimer(checkTopPlayer,1000,0)
