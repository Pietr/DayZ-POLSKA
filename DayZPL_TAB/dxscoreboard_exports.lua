...
stats_account = getAccount ("online_stats") 
if not stats_account then
	stats_account = addAccount ( 'online_stats', "password")
end

totalLogin = getAccountData ( stats_account, "total_login" ) or 0

function askTodayLoginStatsS ()
	triggerClientEvent ( source, "sendTodayLoginStats", root, todayLogin, totalLogin )
end

addEvent("askTodayLoginStats",true)
addEventHandler("askTodayLoginStats",getRootElement(),askTodayLoginStatsS)

addEventHandler("onPlayerLogin", root,
	function()
		todayLogin = todayLogin+1
		totalLogin = totalLogin+1
		setAccountData ( stats_account, "total_login", totalLogin)
		triggerClientEvent ( "sendTodayLoginStats", root, todayLogin, totalLogin )
	end
)

function checkTimeReset ()
	local cur_time = getRealTime()
	if cur_time.hour == 0 then
		todayLogin = 0
		triggerClientEvent ( "sendTodayLoginStats", root, todayLogin, totalLogin )
	end
end
...
function stats()
		todayLogin = todayLogin+1
		setAccountData ( stats_account, "stats", totalLogin)
		triggerClientEvent ( "sendTodayLoginStats", root, todayLogin, totalLogin )

end
addCommandHandler ("statsD+", stats)
function stats1()
		totalLogin = totalLogin+1
		setAccountData ( stats_account, "total_login", totalLogin)
		triggerClientEvent ( "sendTodayLoginStats", root, todayLogin, totalLogin )
...
addCommandHandler ("stats+", stats1)

MAX_PRIRORITY_SLOT = 500

scoreboardColumns = {
	{ ["name"] = "pos", ["width"] = 40, ["friendlyName"] = "ID", ["priority"] = 1 },
	{ ["name"] = "name", ["width"] = 200, ["friendlyName"] = "NICK", ["priority"] = 2 },
	{ ["name"] = "kill", ["width"] = 60, ["friendlyName"] = "Morderstw", ["priority"] = 3 },
	{ ["name"] = "zkill", ["width"] = 80, ["friendlyName"] = "Zabitych zombie", ["priority"] = 4 },
	{ ["name"] = "alive", ["width"] = 80, ["friendlyName"] = "Czas przeżycia", ["priority"] = 5 },
	{ ["name"] = "ping", ["width"] = 40, ["friendlyName"] = "PING", ["priority"] = MAX_PRIRORITY_SLOT },
	{ ["name"] = "fps", ["width"] = 40, ["friendlyName"] = "FPS", ["priority"] = MAX_PRIRORITY_SLOT }
}
resourceColumns = {}
