	...
	for ind, data in pairs(GAC) do
		if (data[1] == group) then
			...
			if (getAccountPlayer(acc)) then
				...
				outputChatBox("[GRUPA] "..message, getAccountPlayer(acc), 9,249,17, true)
...
function doSomethingWithBank(stuff, amount)
	...
	if (not tonumber(amount)) then
		outputChatBox(tostring(amount).. " jest nieznaną kwotą punktów", client, 255, 0, 0)
		return
	end
	...
	if (not group) then
		outputChatBox("Nie jesteś w żadnej grupie", client, 255, 0, 0)
		return
	end
...
function attemptMakeGroup(name)
	...
	if (groupTable[name]) then
		outputChatBox("Istnieje już grupa o takiej nazwie!", client, 255, 0, 0)
		return
	end
	
	local name = string.gsub(name, char, "")
	if (#name > 20) then
		outputChatBox("Nazwa grupo może składać się z max 20 znaków!", client, 255, 0, 0)
		return
	end
	...
	if (not getPlayerGroup(client) or getPlayerGroup(client) == "None" or getPlayerGroup == "nil") then
		...
		setGroup(client, name, date, "Lider")
		outputChatBox("Grupa została utworzona! Nazwa: "..name, client, 0, 255, 0)
	end
end
...
function createGroup(name, creator, date)
	...
	addRank(name, "Lider", toJSON(permissions))
	...
	outputDebugString(getPlayerName(creator).." stworzyl grupe: "..name.." o: "..date)
end
...
function setGroup(player, group, date, rank)
	...
	viewWindow(player)
	outputDebugString(getPlayerName(player).." dolaczyl do grupy: "..group.." jako: "..rank.." o: "..date)
...
function leaveGroup(player)
	...
	if (getPlayerGroupRank(player) == "Lider") then
		outputGroupMessage(getPlayerName(player).." zamknal grupe!", group)
		for ind, data in pairs(GAC) do
			if (data[1] == group) then
...
function kickAccount(account)
	if (not checkGroupAccess(client, 4)) then outputChatBox("Nie masz do tego uprawnień!",client,255,0,0,true) return end
...
function updateMessage(message)
	...
	outputGroupMessage(getPlayerName(client).." zaktualizował wiadomość grupową!", group)
end
...
function makeInvite(player)
	...
	local group = getPlayerGroup(client)
	outputChatBox(getPlayerName(client).." wysłał zaproszenie do dołączenia do grupy: "..group, player, 0, 255, 0)
	outputChatBox("Wysłał zaproszenie "..getPlayerName(player).." z prośbą dołączenia do grupy "..group, client, 0, 255, 0)
...
function accepInvite(group)
	if (not groupTable[group]) then
		outputChatBox(tostring(group).." zwlekałeś zbyt długo!", client, 255, 0, 0)
		return
	end
	setGroup(client, group, _, "Trial")
	outputGroupMessage(getPlayerName(client).." dołączyłdo grupy!", group)
end
...
function warnAccount(account, lvl, reason)
	...
	if (tonumber(mine) <= tonumber(his) and getAccountName(getPlayerAccount(client)) ~= account) then
		outputChatBox("Nie możesz ostrzec tego gracza! Ma wyższą rangę od twojej!", client, 255, 0, 0)
		return
	end
	...
	elseif (tonumber(GAC[account][3] + lvl) >= 100) then
		...
		if (account == getAccountName(getPlayerAccount(client))) then outputChatBox("Nie możesz użyć tego na sobie!", client, 255, 0, 0) return end
		kickAccount(account)
		outputGroupMessage("Konto: "..account.." zostało wyrzucone z grupy przez "..getPlayerName(client).." ("..reason..")", group)
		groupLog(group, "Konto: "..account.." zostało wyrzucone z grupy przez "..getPlayerName(client).." ("..reason..")", group)
		return
	else
	...
	if (online) then
		outputGroupMessage(getPlayerName(online).." został ostrzeżony przez "..getPlayerName(client).." (Level: "..lvl.." (Powód: "..reason..") Ostrzeżeń: "..add..")", group)
		groupLog(group, getPlayerName(online).." został ostrzeżony przez "..getPlayerName(client).." (Level: "..lvl.." (Powód: "..reason..") Ostrzeżeń: "..add..")")
	else
		groupLog(group, "Account: "..account.." został ostrzeżony przez "..getPlayerName(client).." (Level: "..lvl.." (Powód: "..reason..") Ostrzeżeń: "..add..")")
		outputGroupMessage("Account: "..account.." został ostrzeżony przez "..getPlayerName(client).." (Level: "..lvl.." (Powód: "..reason..") Ostrzeżeń: "..add..")", group)
	end
...
function addTheRank(name, selected)
	...
	outputChatBox("Poprawnie dodałeś rangę "..name, client, 0, 255, 0)
...
function editRank(name, newPerm)
	...
	outputChatBox("Zedytowałeś uprawnienia dla rangi '"..name.."'", client, 0, 255, 0)
...
function deleteRank(rank)
	...
	if (getGroupRankCount(group) == 1) then
		outputChatBox("Nie możesz usunąć tej rangi!", client, 255, 0, 0)
		return
	end
	...
	outputGroupMessage(getPlayerName(client).." usunął rangę: "..rank)
...
function setTheRank(rank, account)
	if (not account or not getAccount(account)) then
		outputChatBox("Nie wybrałeś żadnego gracza z listy!", client, 255, 0, 0)
		return
	end
	...
	if (not rankCache[group] or rankCache[group] and not rankCache[group][rank]) then
		outputChatBox("Ranga ("..tostring(rank)..") nie została znaleziona w twojej grupie!", client, 255, 0, 0)
		return
	end
	local online = getAccountPlayer(getAccount(account))
	if (not checkGroupAccess(client, 14)) then outputChatBox("Nie masz do tego uprawnień!",client,255,0,0,true) return end
	if (tonumber(his) > tonumber(mine)) then
		outputChatBox("Nie możesz edytować gracza o wyższej randze w grupie!", client, 255, 0, 0)
		return
	end
	...
	if (GAC[account][2] == rank) then
		outputChatBox("Ten gracz ma już tę rangę "..rank, client, 255, 0, 0)
		return
	end
	if (online) then
		outputGroupMessage(getPlayerName(client).." zmienił rangę "..getPlayerName(online).." na: "..rank, group)
	...
	outputChatBox("Ustawiłeś "..account.." rangę na "..rank, client, 0, 255, 0)
	GAC[account][2] = rank
end
