...
function putAttachedElementsOnBones()
	for element,ped in pairs(attached_ped) do
		...
		elseif isElementStreamedIn(ped) then
			if getElementType (element) == "weapon" and ped ~= localPlayer and getPedControlState ( ped, "aim_weapon" ) then
				...
				if tonumber(tostring(objx)) and tonumber ( tostring(objy) ) and tonumber ( tostring(objz) ) then
					setElementPosition(element,objx,objy,objz)
				end
				if tonumber(tostring(offrx)) and tonumber ( tostring(offry) ) and tonumber ( tostring(offrz) ) then
