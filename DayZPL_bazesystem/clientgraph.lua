...
local windowpl = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Panel użytkownika bazy", false)

guiWindowSetSizable(windowpl, false)
guiSetVisible(windowpl, false)

--guiCreateLabel(0.62, 0.04, 0.4, 0.2, "Notatka:\nJest to panel edycji bazy.\nMożesz tu:\nZmienić nazwę bazy, hasło, zarządzać członkami\n\n", true, windowpl )

arent = guiCreateLabel(0.62, 0.16, 0.4, 0.2, "Dni ważności:\nbrak", true, windowpl )

customTypepl = guiCreateComboBox (0.25, 0.05, 0.35, 0.2, "Wybierz ID bazy", true, windowpl )

guiCreateLabel(0.011, 0.1, 0.20, 0.03, "Nazwa:", true, windowpl )
...
column2pl = guiGridListAddColumn(playergridpl, "Gracz", 0.9)
guiCreateLabel(0.62, 0.81, 0.37, 0.029, "Serial członka:", true, windowpl )
editserialplayerpl = guiCreateEdit(0.62, 0.85, 0.37, 0.06, "", true, windowpl)
addserialpl = guiCreateButton(0.62, 0.93, 0.37, 0.05, "Dodaj", true, windowpl)
delserialpl = guiCreateButton(0.01, 0.93, 0.6, 0.05, "Usuń", true, windowpl)
...
addEventHandler ( "onClientGUIClick", getResourceRootElement(getThisResource()),
	function()
		if source == redactpl then
			if guiComboBoxGetItemText(customTypepl, guiComboBoxGetSelected(customTypepl)) ~= "Wybierz ID bazy" then
				...
				if guiGetText(passeditpl) ~= "" and guiGetText(ownereditpl) ~= "" then
					...
					outputChatBox("Wypełnij wszystkie pola!", 255, 255, 255)
				end
			end
		...
		elseif source == delserialpl then
			if (guiComboBoxGetItemText(customTypepl, guiComboBoxGetSelected(customTypepl)) ~= "Wybierz ID bazy") and (guiGridListGetSelectedItem(playergridlistpl) ~= -1) then
				local id = guiComboBoxGetItemText(customTypepl, guiComboBoxGetSelected(customTypepl))
