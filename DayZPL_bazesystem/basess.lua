...
function createBase(row)
	...
	elseif object == 2779 then
		...
		outputChatBox("Baza posiada niepoprawny obiekt!",source)
		return false
	end
	...
	baseCol = createColCuboid(x-100,y-100,-95-3,200,200,50)
...
function bazedel(source,cmd,id)
	...
	id = tonumber(id)
	if not id then
		outputChatBox("Wprować ID bazy którą chcesz usunąć!",source)
		return false
	elseif id > baseID or id < 0 then
		outputChatBox("Ta baza nie istnieje!",source)
		return false
	end	
	...
	for i, v in ipairs(getElementsByType("colshape")) do
		if getElementData(v,"base") and getElementData(v,"id") == id then
			...
			destroyElement(v)
			outputChatBox("Baza została usunięta.",source)
			return
		end
	end
end
...
function createNewBase(source,cmd,owner,obj,pass,day,namebase)
	...
	if colvo > 40 then
	outputChatBox("Nazwa bazy może mieć maks. 30 znaków!",source, 255, 255, 255) 
	return end
	if not owner or owner == "" then
		outputChatBox("Wpisz serial przyszłego właściciela!",source)
		return false
	end
	if not obj then
		outputChatBox("Wpisz numer bramy!",source)
		return false
	end
	...
	elseif obj == 5 then
		...
		outputChatBox("Niepoprawny obiekt!",source)
		return false
	end
	objj = tonumber(objj)
	...
	local marker = createMarker ( x, y, z-1, "cylinder", 3, 255, 255, 255, 20 ) -- Wejście
	local marker2 = createMarker ( x,y,-95, "cylinder", 3, 255, 255, 255, 20 ) -- Wyjście
...
function redactBase(source,id,owner,pass,day,namebase,obj)
if not (isObjectInACLGroup("user." ..getAccountName(getPlayerAccount(source)), aclGetGroup("Admin"))) then return end
	...
	dbQuery( database, "UPDATE `bases` SET serialowner='"..owner.."', pass='"..pass.."', namebaze='"..namebaze.."', object='"..objj.."', owner='"..namebase.."', day='"..math.floor(getRealTime().timestamp+(86400*(day+1))).."' WHERE id='"..id.."';")
		...
		object = tonumber(baze['object'])
			...
			if isElement(markerr) and isElement(markerr2) then
				if isElementWithinMarker ( source, markerr ) then
					setElementPosition ( getPedOccupiedVehicle ( source ) or source,  baze['x'],baze['y'],-95+3 )
	...
	for index, baze in ipairs(bazas) do
		if baze["serialowner"] ~= "brak" then
			...
			friends = {}
				...
				dbQuery( database, 'UPDATE `bases` SET owner="brak", pass="'..pass..'", human="brak", serialowner="brak" WHERE id="'..baze['id']..'";')
				local id = baze["id"]
				for i, v in ipairs(getElementsByType("colshape")) do
					if getElementData(v,"base") and getElementData(v,"id") == id then
						local marker = getElementData(v,"marker")
				...
				if fromJSON(result2[1].human) then
					for i,v in ipairs(fromJSON(result2[1].human)) do
						...
						if v[1] and serial == v[1] then
							...
							outputChatBox("Wszedłeś.",theElement)
							...
							if not isTimer(alarmTimer[theElement]) then
								alarmTimer[theElement] = setTimer(function()
									...
									outputChatBox("Nie masz dostępu do tej bazy!",theElement)
...
if (isObjectInACLGroup("user." ..getAccountName(getPlayerAccount(source)), aclGetGroup("Admin"))) then
		...
		outputChatBox("Nie możesz tego robić!",source,209,252,115)
	end
end
...
addCommandHandler("baza",openpl)
addCommandHandler("base",openpl)
bindKey ( "F9", "down", openpl )
