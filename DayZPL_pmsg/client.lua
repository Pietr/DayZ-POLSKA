...
function buildPlayerList()
	...
	wndPlayers = guiCreateWindow(-1,256,222,343, "PRWYATNY CHAT", false)
	...
	grdPlayers = guiCreateGridList(0.0991,0.2449,0.8108,0.6968, true, wndPlayers)
		...
		colPlayers = guiGridListAddColumn(grdPlayers, "Lista graczy", 0.85)
...
function buildChatWindow(ply)
	...
	chat[ply].btnClose = guiCreateButton(0.9215,0.0754,0.0557,0.0794, "x", true, chat[ply].wnd)
	chat[ply].btnSend = guiCreateButton(0.757,0.8135,0.2127,0.1429, "Wyslij", true, chat[ply].wnd)
...
guiSetText(newmsg.lbl, "Wiadomosc od\n" .. getPlayerName(ply) .. "\n")
	outputChatBox("#0096ffOtrzymales nowa prywatna wiadomosc! Aby odczytac nacisnij 'F4'!",255,255,255,true)
...
event_resource_start = function(res)
	buildPlayerList()
	outputChatBox("#f80000Nacisnij 'F4' aby pisac prywatne wiadomosci!",255,255,255,true)
end
