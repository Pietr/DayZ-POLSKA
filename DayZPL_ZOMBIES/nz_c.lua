...
checkZombies = function()
	for i,ped in ipairs(getElementsByType("ped",root,true)) do
		if getElementData(ped, "zombie") and not getElementData ( ped, "target" ) then
			...
			local ready = true
			if ( distance > sound*1.2 + visibly*1.2 and scal <= 0 ) or ( distance > sound*1.4 + visibly*1.4 ) then -- Відстань через яку він вас побачить і почує (СЕРЕДНЬО)
				ready = false
			end
			if ready then
...
checkAnimal = function()
	for i,ped in ipairs(getElementsByType("ped",root,true)) do
		if getElementData(ped, "animal") and not getElementData ( ped, "target" ) then
			...
			local visibly = getElementData(getLocalPlayer(), "visibly") / 5
			local ready = true
			if ( distance > sound*3 + visibly*3 and scal <= 0 ) or ( distance > sound*3 + visibly*3 ) then -- Відстань через яку він вас побачить і почує (ДАЛЕКО)
				ready = false 
			end
			if ready then
