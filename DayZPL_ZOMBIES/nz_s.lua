...
function chasedStartAttack (zomb)
	if isElement ( zomb ) and not getElementData ( zomb, "target" ) then
		...
		if getElementData(zomb, "spawn_ztype") == 10 then
			exports.npc_hlc:addNPCTask ( zomb, {"killPed", source, 20, 2} )
			exports.npc_hlc:setNPCWeaponAccuracy ( zomb, 0.87 )
		else
...
function activateZombie (zomb, x, y, z, ztype)
	if isElement ( zomb ) then
		...
		elseif ztype == 10 then
			...
			setElementData(zomb, "speed", "run")			
			giveWeapon(zomb, 31, 160, true) 
			setElementData(zomb, "STANAG", 160)			
			applyStats(zomb)
			--giveWeapon(zomb, 31, 500, true) 
		else
			setElementData ( zomb, "zombie", true )
